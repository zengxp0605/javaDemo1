 ...
 * @Desc:
 */
public class MyApacheDubboServiceBeanListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    // shenyu:
    public MyApacheDubboServiceBeanListener(final MyShenyuRegisterCenterConfig config, final ShenyuClientRegisterRepository shenyuClientRegisterRepository) {
        System.out.println("------>" + config);
        Properties props = config.getProps();
        String contextPath = props.getProperty("contextPath");
        String appName = props.getProperty("appName");
        if (StringUtils.isBlank(contextPath)) {
            throw new RuntimeException("apache dubbo client must config the contextPath or appName");
        }
    ...
    private void handler(final ServiceBean serviceBean) {
        ...
        for (Method method : methods) {
            OpenApiTest shenyuDubboClient = method.getAnnotation(OpenApiTest.class);
    ...
    private MetaDataRegisterDTO buildMetaDataDTO(final ServiceBean serviceBean, final OpenApiTest shenyuDubboClient, final Method method) {
